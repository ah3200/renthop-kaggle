...
def main():
        ...
        for train_index, test_index in kfolder:
                ...
                X1testcv = xgb.DMatrix(csr_matrix(W_cv), label=np.array(y_cv))
                evallist = [(X1, 'train'), (X1testcv, 'test')]
                bst = xgb.train(param.items(), X1, 1000, evallist, early_stopping_rounds=20)
                #predictions
